...
export const buildPromptBuilderCommand = (input: BuildPromptBuilderCommandInput): string => {
  ...
  const cleanedPrompt = primaryTask?.cleanedPrompt?.trim() || planDraft?.cleanedPrompt?.trim() || normalizedTask;
  const rationale = primaryTask?.rationale?.trim() || planDraft?.rationale?.trim() || '';
