...
const normalizePlanDraft = (value: unknown): PlanDraft | null => {
  ...
  return {
    ...
    displayCategory: typeof value.displayCategory === 'string' ? value.displayCategory : (typeof value.taskType === 'string' ? value.taskType : 'general'),
    matchedProfileId: typeof value.matchedProfileId === 'string' ? value.matchedProfileId : null,
    classificationReason: typeof value.classificationReason === 'string' ? value.classificationReason : 'Recovered persisted draft.',
...
const normalizeTaskRoutingProfile = (value: unknown): TaskRoutingProfile | null => {
  ...
  const taskType = typeof value.taskType === 'string' && TASK_TYPES.includes(value.taskType as TaskType)
...
const normalizeRoutingSettings = (value: unknown): RoutingSettings => {
  ...
  const taskProfiles = normalizedProfiles.length > 0
    ? normalizedProfiles
...
const normalizeContinuityState = (value: unknown): RendererContinuityState => {
  ...
  return {
    ...
    selectedPlannedTaskIndex: isNonNegativeInteger(value.selectedPlannedTaskIndex)
      ? value.selectedPlannedTaskIndex
      : 0,
...
const recoverRunsAfterRestart = (appState: PersistedAppData): PersistedAppData => {
  ...
  const runs = appState.runs.map((run) => {
    ...
    didRecover = true;
    const recoveredStatus: 'interrupted' = 'interrupted';
    const recoveredMessage = run.status === 'pending'
  ...
  // Recover orchestration runs that were in-progress
...
const VALID_ORCH_RUN_STATUSES: OrchestrationRunStatus[] = ['planning', 'executing', 'aggregating', 'completed', 'failed', 'cancelled'];
const VALID_ORCH_NODE_STATUSES: OrchestrationNodeStatus[] = ['pending', 'waiting_on_deps', 'ready', 'running', 'completed', 'failed', 'skipped', 'cancelled'];
...
const normalizeRetryPolicy = (value: unknown): RetryPolicy => {
  ...
  return {
    maxRetries: typeof value.maxRetries === 'number' && value.maxRetries >= 0 ? value.maxRetries : DEFAULT_RETRY_POLICY.maxRetries,
    delayMs: typeof value.delayMs === 'number' && value.delayMs >= 0 ? value.delayMs : DEFAULT_RETRY_POLICY.delayMs,
    backoffMultiplier: typeof value.backoffMultiplier === 'number' && value.backoffMultiplier >= 1 ? value.backoffMultiplier : DEFAULT_RETRY_POLICY.backoffMultiplier
  };
};
...
const normalizeAgentProfile = (value: unknown): AgentProfile | null => {
  ...
  const role = typeof value.role === 'string' && VALID_AGENT_ROLES.includes(value.role as AgentRoleType)
  ...
  return {
    ...
    enabledSkillIds: Array.isArray(value.enabledSkillIds) ? (value.enabledSkillIds as string[]).filter((s) => typeof s === 'string') : [],
    enabledMcpServerIds: Array.isArray(value.enabledMcpServerIds) ? (value.enabledMcpServerIds as string[]).filter((s) => typeof s === 'string') : [],
    maxParallelChildren: typeof value.maxParallelChildren === 'number' && value.maxParallelChildren >= 1 ? value.maxParallelChildren : 3,
...
const normalizeSkillDefinition = (value: unknown): SkillDefinition | null => {
  ...
  const trigger = isJsonObject(value.trigger)
    ? {
        keywords: Array.isArray((value.trigger as JsonObject).keywords)
          ? ((value.trigger as JsonObject).keywords as string[]).filter((s) => typeof s === 'string')
          : [],
        taskTypes: Array.isArray((value.trigger as JsonObject).taskTypes)
          ? ((value.trigger as JsonObject).taskTypes as string[]).filter((s) => TASK_TYPES.includes(s as TaskType)) as TaskType[]
          : []
      }
    : { keywords: [], taskTypes: [] };

  const recommendedRole = typeof value.recommendedAgentRole === 'string' && VALID_AGENT_ROLES.includes(value.recommendedAgentRole as AgentRoleType)
...
const normalizeMcpServerDefinition = (value: unknown): McpServerDefinition | null => {
  ...
  const transport = typeof value.transport === 'string' && VALID_MCP_TRANSPORTS.includes(value.transport as McpTransport)
    ? (value.transport as McpTransport)
    : 'stdio';

  const healthStatus = typeof value.healthStatus === 'string' && VALID_MCP_HEALTH.includes(value.healthStatus as McpHealthStatus)
  ...
  return {
    ...
    env: isJsonObject(value.env)
      ? Object.fromEntries(Object.entries(value.env as JsonObject).filter(([, v]) => typeof v === 'string').map(([k, v]) => [k, v as string]))
      : {},
    toolAllowlist: Array.isArray(value.toolAllowlist) ? (value.toolAllowlist as string[]).filter((s) => typeof s === 'string') : [],
    healthStatus,
...
const normalizeOrchestrationRun = (value: unknown): OrchestrationRun | null => {
  ...
  const status = typeof value.status === 'string' && VALID_ORCH_RUN_STATUSES.includes(value.status as OrchestrationRunStatus)
...
const normalizeOrchestrationNode = (value: unknown): OrchestrationNode | null => {
  ...
  const status = typeof value.status === 'string' && VALID_ORCH_NODE_STATUSES.includes(value.status as OrchestrationNodeStatus)
  ...
  return {
    ...
    mcpServerIds: Array.isArray(value.mcpServerIds) ? (value.mcpServerIds as string[]).filter((s) => typeof s === 'string') : [],
    taskType,
    title: typeof value.title === 'string' ? value.title : '',
    prompt: typeof value.prompt === 'string' ? value.prompt : '',
    status,
    ...
    resultPayload: isJsonObject(value.resultPayload) ? (value.resultPayload as Record<string, unknown>) : null,
...
const normalizePersistedAppData = (value: unknown): PersistedAppData | null => {
  ...
  return {
    ...
    mcpServers: Array.isArray(value.mcpServers)
      ? (value.mcpServers as unknown[]).map(normalizeMcpServerDefinition).filter((s): s is McpServerDefinition => s !== null)
      : [],
    orchestrationRuns: Array.isArray(value.orchestrationRuns)
      ? (value.orchestrationRuns as unknown[]).map(normalizeOrchestrationRun).filter((r): r is OrchestrationRun => r !== null)
      : [],
    orchestrationNodes: Array.isArray(value.orchestrationNodes)
      ? (value.orchestrationNodes as unknown[]).map(normalizeOrchestrationNode).filter((n): n is OrchestrationNode => n !== null)
      : []
  };
};
